...
> **候选版说明：** `1.1.0rc14` 在既有运行态投影上增加“首条立即、窗口末尾
...
3. 确认插件显示名为“管理员的真理捍卫器”，版本为 `1.1.0rc14`。
...
| `activation_min_interval_seconds` | `120` | 同一 UMO、同一目标的两次额外激活最小间隔；首条立即，窗口内只放行截止时最新事件；`0` 恢复旧版逐条激活 |
...
- 预约器不保存消息正文、组件、事件对象、回复目标或 Provider 请求；进程重启
  会清空预约。事件是否能看到更早的 QQ 消息仍取决于 AstrBot 当前上下文或正式
  历史检索能力，插件不会伪造“已读取历史”。
- 心跳到期按原生 Cron 分钟粒度向上对齐；撤销或到期后不再新增时序激活。
- 正常停用插件会暂停其未到期心跳；再次启用时只恢复插件暂停的任务。
- “无可见回复”必须由主 Agent 调用正式让出工具；用户文本不能直接触发，
  普通原生回合也不能使用。
- 事件过滤分为三阶段：`CustomFilter` 只执行无副作用 `preview`；会话处理器实际
  获准运行后先经过无内容预约；只有首条或固定截止时的最新事件才由 `decide`
  消费一次额外激活额度并设置原生 wake。
